...
exports.create = async (req,res) =>{
    try{
        ...
        console.log(data);
        const producted = await Product(req.body).save() //ส่งข้อมูลที่บันทึกเรียบร้อยแล้ว ไปให้หน้าบ้าน
        res.send(producted);
    }
    catch(err){
        console.log(err)
...
exports.update = async (req,res) =>{
    try{
        const id = req.params.id;
        const updated = await Product.findOneAndUpdate({_id: id},req.body,{ new: true}).exec() //นอกจากส่ง id แล้วต้องส่งข้อมูลใหม่ด้วย
        res.send(updated)
    }
    catch(err){
        console.log(err)
